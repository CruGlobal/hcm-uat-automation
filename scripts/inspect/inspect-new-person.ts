...
(async () => {
  ...
  // Open Navigator and click "New Person" directly
  ...
  // Click "New Person" under My Team
  console.log('Clicking New Person...');
  await page.locator('a[title="New Person"]').click();
  await page.waitForLoadState('networkidle', { timeout: 60000 });
  await page.waitForTimeout(5000);
  console.log('New Person URL:', page.url());
  await page.screenshot({ path: '/tmp/new-person.png', fullPage: true });

  // Dump all visible elements
  const els = await page.locator('input, button, select, label, h1, h2, h3, [role="heading"], [role="button"], [role="combobox"], [role="listbox"]').evaluateAll((els: any[]) =>
  ...
  console.log('New Person elements:', JSON.stringify(els, null, 2));
