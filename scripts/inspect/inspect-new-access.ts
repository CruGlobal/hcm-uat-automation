...
(async () => {
  ...
  await page.screenshot({ path: '/tmp/new-person-updated.png', fullPage: true });

  // Dump all task links on the page
  const tasks = await page.locator('a[title]').evaluateAll((els: any[]) =>
    els.filter((el: any) => el.offsetParent !== null && el.offsetWidth > 0 && el.title && !['Navigator','Home','Back','Close','Show More','Settings and Actions','Access Accessibility Settings','Oracle Logo Home','Help'].includes(el.title)).map((el: any) => ({
